...
RUserUi::AddOthersWindow::AddOthersWindow(mongocxx::database db)
{
...
int RUserUi::AddOthersWindow::others_current_item = 0;
char RUserUi::AddOthersWindow::other_name[128] = "";

int RUserUi::AddOthersWindow::create_add_others_window()
{
...
RUserUi::RemoveOthersWindow::RemoveOthersWindow(mongocxx::database db)
{
    this->products_collection = db["Products"];
    // Initialize p_others
    this->p_others = std::make_unique<char[]>(this->others.size() + 1);
...
int RUserUi::RemoveOthersWindow::r_others_current_item = 0;
int RUserUi::RemoveOthersWindow::r_other_selected_current_item = 0;

int RUserUi::RemoveOthersWindow::create_remove_others_window()
{
    // Professional styling
    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.92f, 0.93f, 0.94f, 1.0f)); // Soft off-white text
    ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4(0.15f, 0.16f, 0.18f, 1.0f)); // Deep slate gray background
...
int RUserUi::RemoveOthersWindow::mset_others(std::vector<std::string> v_s_others)
{
...
RUserUi::AddorRemoveOthersWindow::AddorRemoveOthersWindow(AddOthersWindow& add_other, RemoveOthersWindow& remove_other)
{
    this->add_other = &add_other;
    this->remove_other = &remove_other;
}

int RUserUi::AddorRemoveOthersWindow::create_add_or_remove_others_window()
{
    // Professional styling
    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.92f, 0.93f, 0.94f, 1.0f)); // Soft off-white text
    ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4(0.15f, 0.16f, 0.18f, 1.0f)); // Deep slate gray background
    ...
    if (this->add_other->msuccess)
    {
        this->add_other->notify_sale_status(this->add_other->msuccess);
        this->add_other->show_window = false;
        this->add_other->msuccess = false;
    }

    if (this->remove_other->show_window)
        this->remove_other->create_remove_others_window();

    if (this->remove_other->msuccess)
    {
        this->remove_other->notify_sale_status(this->remove_other->msuccess);
        this->remove_other->show_window = false;
        this->remove_other->msuccess = false;
    }
