...
RUserUi::AddorRemoveItemWindow::AddorRemoveItemWindow(AddItemWindow& add_item, RemoveItemWindow& remove_item)
{
	this->add_item    = &add_item;
	this->remove_item = &remove_item;
}

int RUserUi::AddorRemoveItemWindow::create_add_or_remove_items_window()
{
...
RUserUi::AddNewBrandWindow::AddNewBrandWindow(mongocxx::database db)
{
...
int RUserUi::AddNewBrandWindow::create_add_new_brand_window()
{
    // Handle any error
    if (this->brand_win.minput_error == true)
        this->brand_win.admin_error_message(this->brand_win.minput_error);
...
RUserUi::AddNewQuantityWindow::AddNewQuantityWindow(mongocxx::database db)
{
...
int RUserUi::AddNewQuantityWindow::create_add_new_quantity_window()
{
    ...
    // Handle any error
    if (this->quantity_win.minput_error == true)
        this->admin_error_message(this->quantity_win.minput_error);

    return 1;
}

RUserUi::AddItemWindow::AddItemWindow(AddProductWindow& add_product, AddNewBrandWindow& add_brand, AddNewQuantityWindow& add_quantity)
{
...
int RUserUi::AddItemWindow::create_add_items_window()
{
    ...
    if (this->add_product->msuccess == true)
        this->add_product->notify_sale_status(this->add_product->msuccess);
    ...
    if (this->add_brand->brand_win.msuccess == true)
        this->add_brand->brand_win.notify_sale_status(this->add_brand->brand_win.msuccess);

    if (this->add_quantity->quantity_win.show_window)
        this->add_quantity->create_add_new_quantity_window();

    if (this->add_quantity->quantity_win.msuccess == true)
        this->add_quantity->notify_sale_status(this->add_quantity->quantity_win.msuccess);
...
RUserUi::RemoveItemWindow::RemoveItemWindow(RemoveProductWindow& remove_product, RemoveBrandWindow& remove_brand, RemoveQuantityWindow& remove_quantity)
{
...
int RUserUi::RemoveItemWindow::create_remove_items_window()
{
    // Push UX-friendly colors
    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.95f, 0.95f, 0.95f, 1.0f)); // Near-white text for high contrast
    ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4(0.13f, 0.14f, 0.15f, 1.0f)); // Dark charcoal background
    ...
    if (this->remove_product->msuccess == true)
    {
        this->remove_product->notify_sale_status(this->remove_product->msuccess);
    ...
    if (this->remove_brand->msuccess == true)
    {
        this->remove_brand->notify_sale_status(this->remove_brand->msuccess);
    ...
    if (this->remove_quantity->msuccess == true)
    {
        this->remove_quantity->notify_sale_status(this->remove_quantity->msuccess);
