...
int delete_location(mongocxx::collection products, std::string location)
{
    ...
    {
        brands = product_view["Brands"][std::string(b.get_string().value)].get_array();

        for (bsoncxx::array::element x : brands)
        {
            quantities = product_view["Quantities"][std::string(x.get_string().value)].get_array();
            ...
            for (bsoncxx::array::element z : quantities)
            {
                ...
                for (bsoncxx::array::element q : seasons)
                {
                    ...
                    for (bsoncxx::array::element g : age_groups)
                    {
                        ...
                        {
                            nav_path = "Brands." + std::string(x.get_string().value) + "." + std::string(z.get_string().value) + "." +
                                std::string(q.get_string().value) + "." + std::string(g.get_string().value) + "." + std::string(a.get_string().value) + "." + location;

                                products.update_one(make_document(kvp("name", std::string(b.get_string().value))), make_document(kvp("$unset", make_document(kvp(nav_path, "")))));
                        };
                    };
                };
            };
    ...
    for (bsoncxx::array::element b : prodtcs)
    {
        ...
        for (bsoncxx::array::element x : brands)
        {
            ...
            for (bsoncxx::array::element z : quantities)
            {
                ...
                {
                   nav_path = "Brands." + std::string(x.get_string().value) + "." + std::string(z.get_string().value) + "." +
                       std::string(q.get_string().value) + "." + age_group;
...
int delete_season(mongocxx::collection products, std::string season)
{
    ...
    for (bsoncxx::array::element b : prodtcs)
    {
        ...
        for (bsoncxx::array::element x : brands)
        {
            ...
            for (bsoncxx::array::element z : quantities)
            {
                    nav_path = "Brands." + std::string(x.get_string().value) + "." + std::string(z.get_string().value) + "." +  season;
