...
int reset_stock(mongocxx::collection products, mongocxx::collection stock, SALE sale, MODE mMode)
{
    ...
    {
        stock_products = stock.find_one(document{} << "name" << std::string(p.get_string().value) << finalize);
        stock_products_view = stock_products.value().view();

        sale.product_name = std::string(p.get_string().value);
        update_stock_product_total(stock, stock_products_view, sale, mMode);

        brands = components_arrays_view["Brands"][std::string(p.get_string().value)].get_array();

        for (bsoncxx::array::element b : brands)
        {
            sale.brand = std::string(b.get_string().value);
            update_stock_brand_total(stock, stock_products_view, sale, mMode);

            quantities = components_arrays_view["Quantities"][std::string(b.get_string().value)].get_array();

            for (bsoncxx::array::element q : quantities)
            {
                sale.quantity  = std::string(q.get_string().value);
...
int update_Tbuying_price_during_brand_removal(mongocxx::collection products, mongocxx::collection stock, std::string product_name, std::string brand_name)
{
    ...
    {
        current_amount = stock_products_view["Brands"][brand_name][std::string(q.get_string().value)]["Total"];
        number_of_items = current_amount.get_int32();

        buying_price = stock_products_view["Brands"][brand_name][std::string(q.get_string().value)]["BuyingPrice"];
