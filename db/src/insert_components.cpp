...
int insert_location(mongocxx::collection products, std::string location)
{
	...
	{
		_brands = mview["Brands"][std::string(p.get_string().value)].get_array();
		for (const bsoncxx::array::element& b : _brands)
		{
			_quantities = mview["Quantities"][std::string(b.get_string().value)].get_array();
			for (const bsoncxx::array::element& q : _quantities)
			{
				...
				for (const bsoncxx::array::element& s : _seasons)
				{
					...
					for (const bsoncxx::array::element& a : _age_group)
					{
						...
						{
							_path = "Brands." + std::string(b.get_string().value) + "." +
								std::string(q.get_string().value) + "." +
								std::string(s.get_string().value) + "." +
								std::string(a.get_string().value) + "." +
								std::string(g.get_string().value) + "." + location;
   							products.update_one(make_document(kvp("name", std::string(p.get_string().value))), make_document(kvp("$set", make_document(kvp(_path, 0)))));
						}
					}
				}
			}
				...
				for (const bsoncxx::array::element& s : _seasons)
				{
					...
					for (const bsoncxx::array::element& a : _age_group)
					{
						_path = "Brands." + std::string(b.get_string().value) + "." +
							std::string(q.get_string().value) + "." +
							std::string(s.get_string().value) + "." +
							std::string(a.get_string().value) + "." + gender + "." + "Total";
						products.update_one(make_document(kvp("name", std::string(p.get_string().value))), make_document(kvp("$set", make_document(kvp(_path, 0)))));
						...
						for (const bsoncxx::array::element& l : _locations)
						{
							_path = "Brands." + std::string(b.get_string().value) + "." +
								std::string(q.get_string().value) + "." +
								std::string(s.get_string().value) + "." +
								std::string(a.get_string().value) + "." + gender + "." + std::string(l.get_string().value);
							products.update_one(make_document(kvp("name", std::string(p.get_string().value))), make_document(kvp("$set", make_document(kvp(_path, 0)))));
		...
		for (const bsoncxx::array::element& b : _brands)
		{
			...
			for (const bsoncxx::array::element& q : _quantities)
			{
				...
				for (const bsoncxx::array::element& s : _seasons)
				{
					_path = "Brands." + std::string(b.get_string().value) + "." +
						std::string(q.get_string().value) + "." +
						std::string(s.get_string().value) + "." + age_group + "." + "Total";
					products.update_one(make_document(kvp("name", std::string(p.get_string().value))), make_document(kvp("$set", make_document(kvp(_path, 0)))));
					...
					for (const bsoncxx::array::element& g : _genders)
					{
						_path = "Brands." + std::string(b.get_string().value) + "." +
							std::string(q.get_string().value) + "." +
							std::string(s.get_string().value) + "." + age_group + "." + std::string(g.get_string().value) + "." + "Total";
						products.update_one(make_document(kvp("name", std::string(p.get_string().value))), make_document(kvp("$set", make_document(kvp(_path, 0)))));
						...
						for (const bsoncxx::array::element& l : _locations)
						{
							_path = "Brands." + std::string(b.get_string().value) + "." +
								std::string(q.get_string().value) + "." +
								std::string(s.get_string().value) + "." + age_group + "." + std::string(g.get_string().value) + "." + std::string(l.get_string().value);
							products.update_one(make_document(kvp("name", std::string(p.get_string().value))), make_document(kvp("$set", make_document(kvp(_path, 0)))));
	...
	for (const bsoncxx::array::element& p : _products)
	{
		...
		for (const bsoncxx::array::element& b : _brands)
		{
			...
			{	
				...
				_path = "Brands." + std::string(b.get_string().value) + "." + std::string(q.get_string().value) + "." + my_season +  "." + "Total";
				products.update_one(make_document(kvp("name", std::string(p.get_string().value))), make_document(kvp("$set", make_document(kvp(_path, 0)))));

				_age_group = mview["Age Groups"].get_array();
				for (const bsoncxx::array::element& a : _age_group)
				{
					_path = "Brands." + std::string(b.get_string().value) + "." + std::string(q.get_string().value) + "." + my_season + "." + std::string(a.get_string().value) +  "." + "Total";
					products.update_one(make_document(kvp("name", std::string(p.get_string().value))), make_document(kvp("$set", make_document(kvp(_path, 0)))));

					_genders = mview["Genders"].get_array();
					for (const bsoncxx::array::element& g : _genders)
					{
						_path = "Brands." + std::string(b.get_string().value) + "." + std::string(q.get_string().value) + "." + my_season + "." + std::string(a.get_string().value) + "." + std::string(g.get_string().value) + "." + "Total";
						products.update_one(make_document(kvp("name", std::string(p.get_string().value))), make_document(kvp("$set", make_document(kvp(_path, 0)))));
						...
						{
							_path = "Brands." + std::string(b.get_string().value) + "." +
								std::string(q.get_string().value) + "." + my_season + "." + std::string(a.get_string().value) + "." + std::string(g.get_string().value) + "." + std::string(l.get_string().value);
							products.update_one(make_document(kvp("name", std::string(p.get_string().value))), make_document(kvp("$set", make_document(kvp(_path, 0)))));
						}
					}
				}
...
int insert_quantity(mongocxx::collection products, mongocxx::collection stock, std::string product_name, std::string brand_name, std::string quantity, double selling_price, double buying_price, int num_of_items)
{
	...
	for (const bsoncxx::array::element& s : _seasons)
	{
		_path = "Brands." + product_name + "_" + brand_name + "." + quantity + "." + std::string(s.get_string().value) + "." + "Total";
		products.update_one(make_document(kvp("name", product_name)), make_document(kvp("$set", make_document(kvp(_path, 0)))));

		_age_group = mview["Age Groups"].get_array();
		for (const bsoncxx::array::element& a : _age_group)
		{
			_path = "Brands." + product_name + "_" + brand_name + "." + quantity + "." + std::string(s.get_string().value) + "." + std::string(a.get_string().value) + "."  + "Total";
			products.update_one(make_document(kvp("name", product_name)), make_document(kvp("$set", make_document(kvp(_path, 0)))));

			_genders = mview["Genders"].get_array();
			for (const bsoncxx::array::element& g : _genders)
			{
				_path = "Brands." + product_name + "_" + brand_name + "." + quantity + "." + std::string(s.get_string().value) + "." + std::string(a.get_string().value) + "." + std::string(g.get_string().value) + "." + "Total";
				products.update_one(make_document(kvp("name", product_name)), make_document(kvp("$set", make_document(kvp(_path, 0)))));
				...
				for (const bsoncxx::array::element& l : _locations)
				{
					_path = "Brands." + product_name + "_" + brand_name + "." + quantity + "." +
						std::string(s.get_string().value) + "." + std::string(a.get_string().value) + "." + std::string(g.get_string().value) + "." + std::string(l.get_string().value);
					products.update_one(make_document(kvp("name", product_name)), make_document(kvp("$set", make_document(kvp(_path, 0)))));
				}
			}
		}
...
int insert_brand(mongocxx::collection products, mongocxx::collection stock, std::string brand, std::vector<std::string> quantities, std::vector<double> selling_price, std::vector<double> buying_price, std::vector<int> number_of_items,  std::string product_name)
{
	...
	for (std::string q : quantities)
	{
		...
		for (const bsoncxx::array::element& s : _seasons)
		{
			_path = "Brands." + brand + "." + q + "." + std::string(s.get_string().value) + "." + "Total";
			products.update_one(make_document(kvp("name", product_name)), make_document(kvp("$set", make_document(kvp(_path, 0)))));

			_age_group = mview["Age Groups"].get_array();
			for (const bsoncxx::array::element& a : _age_group)
			{
				_path = "Brands." + brand + "." + q + "." + std::string(s.get_string().value) + "." + std::string(a.get_string().value) + "." + "Total";
				products.update_one(make_document(kvp("name", product_name)), make_document(kvp("$set", make_document(kvp(_path, 0)))));

				_genders = mview["Genders"].get_array();
				for (const bsoncxx::array::element& g : _genders)
				{
					_path = "Brands." + brand + "." + q + "." + std::string(s.get_string().value) + "." + std::string(a.get_string().value) + "." + std::string(g.get_string().value) + "." + "Total";
					products.update_one(make_document(kvp("name", product_name)), make_document(kvp("$set", make_document(kvp(_path, 0)))));

					_locations = mview["Locations"].get_array();
					for (const bsoncxx::array::element& l : _locations)
					{
						_path = "Brands." + brand + "." + q + "." + std::string(s.get_string().value) + "." + std::string(a.get_string().value) + "." + std::string(g.get_string().value) + "." + std::string(l.get_string().value);
						products.update_one(make_document(kvp("name", product_name)), make_document(kvp("$set", make_document(kvp(_path, 0)))));
					}
				}
			}
	...
	{		
		...
		for (std::vector<std::string> q : qty_names)
		{
			for(std::string qty : q)
			{
				...
				for (const bsoncxx::array::element& s : _seasons)
				{
					_path = "Brands." + b + "." + qty + "." + std::string(s.get_string().value) + "." + "Total";
					products.update_one(make_document(kvp("name", product_name)), make_document(kvp("$set", make_document(kvp(_path, 0)))));

					_age_group = mview["Age Groups"].get_array();
					for (const bsoncxx::array::element& a : _age_group)
					{
						_path = "Brands." + b + "." + qty + "." + std::string(s.get_string().value) + "." + std::string(a.get_string().value) + "."  + "Total";
						products.update_one(make_document(kvp("name", product_name)), make_document(kvp("$set", make_document(kvp(_path, 0)))));

						_genders = mview["Genders"].get_array();
						for (const bsoncxx::array::element& g : _genders)
						{
							_path = "Brands." + b + "." + qty + "." + std::string(s.get_string().value) + "." + std::string(a.get_string().value) + "." +	std::string(g.get_string().value) + "." + "Total";
							products.update_one(make_document(kvp("name", product_name)), make_document(kvp("$set", make_document(kvp(_path, 0)))));
							...
							for (const bsoncxx::array::element& l : _locations)
							{
								_path = "Brands." + b + "." +
									...
									std::string(s.get_string().value) + "." +
									std::string(a.get_string().value) + "." +
									std::string(g.get_string().value) + "." + std::string(l.get_string().value);
								products.update_one(make_document(kvp("name", product_name)), make_document(kvp("$set", make_document(kvp(_path, 0)))));
							}
