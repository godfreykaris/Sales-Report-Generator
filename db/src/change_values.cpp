...
int reset_sales(mongocxx::collection products, SALE sale, MODE mMode)
{
    ...
    {
        complete_sales = products.find_one(document{} << "name" << std::string(p.get_string().value) << finalize);
        ...
        for (bsoncxx::array::element b : brands)
        {
            ...
            for (bsoncxx::array::element q : quantities)
            {
                ...
                {
                    sale.season = std::string(s.get_string().value);
                    ...
                    {
                        sale.age_group = std::string(a.get_string().value);
                        ...
                        {
                            sale.gender = std::string(g.get_string().value);
                            ...
                            {                                 
                                sale.location = std::string(l.get_string().value);
